...
def main(): 
    ...
    if not args.exp_name:
        # text_augment_info = f'istextaug{args.is_text_augment}_pool{args.n_text_para_pool}_KNN(1-11)_numpara{args.n_text_paras}'
        # img_augment_info = f'isimgaug{args.is_img_augment}_UNIQUE_LayerNormForAll'
        # img_augment_info = f'isimgaug{args.is_img_augment}'
        # text_augment_info = f'istextaug{args.is_img_augment}'
        # exp_name = f'Teacher_curr(1-0)_{text_augment_info}_{img_augment_info}'
        exp_name = f'Model_AugText&Img_Curr'
    else:
    ...
    teacher_save_best_path = f'./{weights_dirname}/{exp_name}_teacher_best.pt'
    
    teacher = ViVQAModel(projection_dim=args.projection_dim, 
    ...
    # Load training dataset
    train_loss_lst, train_acc_lst, train_cider_lst, val_loss_lst, val_acc_lst, val_cider_lst = train(teacher, 
                                                                                                    train_loader, 
                                                                                                    test_loader, 
                                                                                                    epochs=args.epochs, 
                                                                                                    criterion=criterion, 
                                                                                                    ...
                                                                                                    save_best_path=teacher_save_best_path,
    ...
    best_teacher = ViVQAModel(projection_dim=args.projection_dim,
                                    hidden_dim=args.hidden_dim,
                                    answer_space_len=answer_space_len,
                                    text_encoder_dict=text_encoder_dict, 
                                    img_encoder_dict=img_encoder_dict,
    ...
    checkpoint = torch.load(teacher_save_best_path,
                        weights_only=True,
    ...
    test_loss, test_acc, test_cider = evaluate(model=best_teacher,
