...
class ViVQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_mode == 'train' and self.is_img_augment:
            aug_imgs_folderpath = os.path.join(self.aug_imgs_rootpath, str(img_ids[:-4]))
            augmented_imgs_pil = augment_image_multi_unique(img_pils, aug_imgs_folderpath)
            augmented_imgs = [self.img_encoder_dict['img_processor'](img) for img in augmented_imgs_pil]

            img_inputs_lst += augmented_imgs 
            # print(f"Number of images in the batch: {len(img_inputs_lst)}")
