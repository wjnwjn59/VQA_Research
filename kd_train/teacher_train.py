...
def main(): 
    ...
    # Load training dataset
    train_loss_lst, train_acc_lst, val_loss_lst, val_acc_lst = train(teacher, 
                                                                    train_loader, 
                                                                    test_loader, 
                                                                    epochs=args.epochs, 
                                                                    criterion=criterion, 
    ...
    best_teacher_model = ViVQAModel(projection_dim=args.projection_dim,
                                    hidden_dim=args.hidden_dim,
                                    answer_space_len=answer_space_len,
                                    text_encoder_dict=text_encoder_dict, 
                                    img_encoder_dict=img_encoder_dict,
    ...
    best_teacher_model.load_state_dict(checkpoint['model']) 
    
    # Evaluate the model on the test set
    teacher_test_loss, teacher_test_acc = evaluate(model=best_teacher_model,
                                                    val_loader=test_loader,
                                                    criterion=criterion)
    ...
    exp_table = wandb.Table(
        ...
        data=[list(args_dict.values()) + [teacher_test_loss, teacher_test_acc]]
    )
